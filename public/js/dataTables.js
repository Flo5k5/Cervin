...
if(oPaging.iPage===oPaging.iTotalPages-1||oPaging.iTotalPages===0){$('li:last',an[i]).addClass('disabled');}else{$('li:last',an[i]).removeClass('disabled');}}}}});jQuery.fn.dataTableExt.oApi.fnFilterOnReturn=function(oSettings){var _that=this;this.each(function(i){$.fn.dataTableExt.iApiIndex=i;var anControl=$('input',_that.fnSettings().aanFeatures.f);anControl.unbind('keyup').bind('keypress',function(e){if(e.which==13){$.fn.dataTableExt.iApiIndex=i;_that.fnFilter(anControl.val());}});return this;});return this;};var ResultSet=new function(){return{paginate:function(values,callback){var params={"sDom":"<'row-fluid'<'span6'l><'span6'f>r>t<'row-fluid'<'span6'i><'span6'p>>","sPaginationType":"bootstrap","oLanguage":{"oPaginate":{"sFirst":"Premier","sNext":"Prochain","sPrevious":"Précédent","sLast":"Dernier"},"sLengthMenu":"Afficher _MENU_ éléments par page","sZeroRecords":"Aucun résultat trouvé","sInfo":"Affiche de _START_ à _END_ sur _TOTAL_ éléments","sInfoEmpty":"Aucun enregistrement à afficher","sInfoFiltered":"(filtrés à partir de _MAX_ entrées)","sSearch":"Recherche ","sEmptyTable":"La table est vide","sLoadingRecords":"Chargement...","sProcessing":"En cours de traitement..."}};if(typeof values!='undefined'){for(var i in values){if(values.hasOwnProperty(i)){params[i]=values[i];}}}
var data=$('#data').dataTable(params).fnFilterOnReturn();if(typeof callback=="function"){callback();}
return data;}};};